...
class ApiUseCaseTest {
    ...
    fun successCase() = runTest {
        ...
        val apiUseCase = ApiUseCase(repository = repository)
        apiUseCase().collect {
            when (it) {
    ...
    fun failureCase() = runTest {
        val repository = mockk<Repository>()
        coEvery { repository.getItems() } returns flowOf(Result.Loading, Result.Error(error = DataError.NetworkError.NOT_FOUND))
        val apiUseCase = ApiUseCase(repository = repository)
        apiUseCase().collect {
            when (it) {
                is Result.Loading -> assertEquals(expected = Result.Loading, actual = it)
                ...
                is Result.Error -> assertEquals(
                    expected = DataError.NetworkError.NOT_FOUND,
                    actual = it.error
                )
            }
        }
